...
function AppGen(opt) {
  ...
  var baseopt = {
    macro: {
      import:{steps:[
        { name:'find', paths:'${arg.path||arg.from}', exclude:'${arg.exclude||""}'},
        { name:'for', opt:{
          each:'${gen.paths}',
          steps:[
            { name:'load', path:'${gen.item}'},
            { name:'save',  opt:{path:'${arg.save||(arg.to||gen.resource.folder)+"/"+gen.resource.name}'}},
          ]                     
        }},
      ]},
        ...
        { name:'for', opt:{
          each:'${gen.paths}',
            steps:[
              ...
              { name:'save',  opt:{path:'${arg.save||gen.resource.folder+"/"+gen.resource.name}'}},
            ]                     
        }},
      ]},
    }
