...
function AppGen(opts) {
  ...
  var errhandler = function( ctxt, err, cb ) {
  ...
  self.exec = function() {
    ...
    function nextcond( ctxt, cb ) {
      ...
      if( !cond ) return nextstep( ctxt, cb );

      var err
      try {
        err = cond.exec( ctxt.api, function(err,res) {
          if( err ) { return errhandler(ctxt,err,cb) }
        ...
        return errhandler(ctxt,ex,cb)
      }

      if( err ) {
        return errhandler(ctxt,err,cb)
      }
    }


    function nextstep( ctxt, cb ) {
