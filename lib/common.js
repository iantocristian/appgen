...
var interpolate = exports.interpolate = function( str, spec, mode, ctxt, cb ) {
  ...
  function nextmatch(whence) {
    ...
    //console.log(i+': mlast='+mlast+' sb='+sb)
    ...
    if( '#'===q.charAt(0) ) {
      if( mode.resource ) {
        ...
        console.log('load path='+respath)
        ctxt.resman.load({path:respath},function(err,text){
          console.log('load err='+err+' text='+text)
      ...
      if( mode.required ) {
        if( val === void 0 ) {
          return onerr( new Error('query "'+m[1]+'" has no matches') )
        }
      }

      sb.push(val)
      return nextmatch('ddd')
    }
  }

  return nextmatch('eee')
}
