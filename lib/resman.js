...
function FileResMan() {
  ...
  self.copy = function(opt,cb) {
    opt.path = self.canon( fspath.normalize( opt.path ) )
    opt.savepath = self.canon( opt.savepath )
    ...
    fs.mkdir(self.gen,function(err){
      ...
      fs.mkdir(self.out,function(err){
        ...
        var parts = (opt.savepath.split('/').slice(0,-1))
        mkdir(1)
        function mkdir(i){
          if( i <= parts.length ) {
            var folder = self.out+'/'+parts.slice(0,i).join('/')
        ...
        function copyFile() {
          var savepath = self.out+'/'+opt.savepath;
          ...
          fspath.exists(savepath, function(exists) {
            if (exists) {
  ...
  self.api = {
    ...
    copy: common.delegate(self,self.copy),
    mkdir: common.delegate(self,self.mkdir),
    ctxt: common.delegate(self,self.ctxt),
  }
