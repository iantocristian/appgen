...
function FileResMan() {
  ...
  self.fork = function( ctxt ) {
    ...
    fork.base = fspath.normalize(ctxtres.base)
    fork.gen  = fspath.normalize(ctxtres.gen)
    fork.out  = fspath.normalize(ctxtres.gen+'/'+ctxt.name)
  ...
  self.load = function(opt,cb) {
    var path = fspath.normalize( opt.path )
    ...
    fs.readFile(path,encoding, function(err,text){
      ...
      var relativePath = fspath.normalize( fspath.dirname(opt.path) )
      relativePath = relativePath.replace(/\\/g, '/')

      relativePath += '/'
      relativePath = relativePath.replace(opt.from, '')
      if (relativePath != '') relativePath = relativePath.slice(0, -1)
