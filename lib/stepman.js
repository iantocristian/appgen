...
function SaveStep( opts ) {
  ...
  self.exec = function( ctxt, cb ) {
    ...
    if( !text && src) {
      text = ctxt.resman.load({path:src},interpolate)
    }
    else interpolate(null,text);

    function interpolate( err, text ) {
      if( err ) return cb(err);

      if( !opts.verbatim.path ) {
        path = common.interpolate( path, ctxt.spec, {required:1} )
      }

      console.log('path='+path)

      if( !opts.verbatim.text ) {
        common.interpolate( text, ctxt.spec, {required:1,resource:1}, ctxt, function( err, text ){
          console.log('t err='+err+' path='+path)
