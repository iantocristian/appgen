...
function SaveStep( opts ) {
  ...
  self.exec = function( ctxt, cb ) {
    ...
    if( !text && src) {
      ...
      ctxt.resman.load({path:loadsrc},function(err,reslist){
        ...
        // FIX: serialize and halt on error
        reslist.each( function( err, text, srcpath, srcbase ) {
          if( !text ) { return cb(err) }
          interpolate(err,text,srcpath)
        })
      })
    }
    else interpolate(null,text,path);

    function interpolate( err, text, srcpath, srcbase ) {
      if( err ) return cb(err);


      //console.log('path='+path+', srcpath='+srcpath )

      if( !opts.verbatim.text ) {
        var spec = _.clone(ctxt.spec)
        spec.res.$name = fspath.basename(srcpath)
        //console.dir(spec)
        common.interpolate( text, spec, {required:1,resource:1}, ctxt, function( err, text ){

          var savepath = path
          if( !opts.verbatim.path ) {
            savepath = common.interpolate( path, spec, {required:1} )
          }

          //console.log('t err='+err+' savepath='+savepath)
          //console.log('t text='+text)
          save(err,savepath,text)
        })
      }
    }

    function save(err,path,text) {
      //console.log('save err='+err+' path='+path)
      if( err ) return cb(err);
      ctxt.resman.save({path:path,text:text}, function(err){
        if( !text ) { return cb(err) }
      })
    }
  }
